...
# Since func are_all_cal_for_a_material_complete is only called in 
# def are_all_cal_for_a_material_complete(mat_folder, cal_name_list, workflow):
#     """
#     input arguements:
...
def check_calculations_status(cal_folder, workflow, mat_folder_name_list=None, cal_loc_list=None, ignore_complete=False):
    """
    ...
    input argument:
        ...
        - ignore_complete (boolean, default: False): whether to ignore signal file __complete__. It is deactivated if cal_loc_list is provided.
    ...
    signal_file_list = ["__done__",  "__done_cleaned_analyzed__", "__done_failed_to_clean_analyze__", "__manual__", "__test__", "__vis__", 
                        "__skipped__", "__stop__", "__ready__", "__prior_ready__", "__sub_dir_cal__", "__error__", "__running__",  "__killed__", "__nkx_gt_ikptd__"]
    ...
    if cal_loc_list != None:
        ...
        for mat_folder in mat_folder_name_list:
            ...
            if os.path.isdir(mat_folder):
                if ignore_complete == False and os.path.isfile(os.path.join(mat_folder, "__complete__")):
    ...
    for directory in directory_list:
        ...
        # root.rglob("*") does not include root itself.
        candidates = [root]
        ...
        for subdir in candidates:
            ...
            # must be under a directory whose name starts with "step_"
    ...
    # For debug
    #print(f"job_list={job_list}", flush=True)
