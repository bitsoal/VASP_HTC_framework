...
def choose_ispin_based_on_prev_cal(current_cal_loc, prev_cal_step, mag_threshold, workflow):
    ...
    if prev_ispin == 1:
        with open(os.path.join(current_cal_loc, "log.txt"), "a") as log_f:
            ...
            log_f.write("\t\t\tNevertheless, let's assume the total magnetic moment of the previous step is 0 and proceed with the input file preparation.\n")
            log_f.write("\t\t\tCreate file __non_spin_polarized_prev_cal__ as a warning signal for you to check if this is what you want\n")
