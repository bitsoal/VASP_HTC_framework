...
* Asst/Prof. [Yang Ming](https://www.polyu.edu.hk/ap/people/academic-staff/prof-yang-ming/) (co-supervisor)  
...
* **Sub-folder calculations**
  * **Convergence test of essential paramters (ENCUT, SIGMA and KPOINTS)**
...
## How to write up `HTC_calculation_setup_file`
...
* In a firework
  ...
  * Note that **in the first firework**, some job management tags must be set and these setting will be used for the whole workflow:
    ...
    * tag `2d_system`: for 2D systems, KPOINTS will be modified so that `K_z` is zero for all kpoints.*(obsolete in the near future.)*
    * tag `sort_structure`: Whether sort structure or not beforing write POSCAR by the electronegativity of the species 
    * tag `max_running_job` and `job_name`: they decide the maximum number of running/pending jobs.
    * tag `max_no_of_ready_jobs`: The maximum number of jobs that are prepared and ready to run. (See below for more information about this tag)  
...
kpoints_type = MPRelaxSet #Just set this here. Will be deprecated soon
...
## Built-in Tag list - below are built-in tags that can be directly used in `HTC_calculation_setup_file`
...
## Tag list - below are tags that can be set in `HTC_calculation_setup_file`
...
- `structure_folder` (str), **required for the first firework**  
...
- `cal_folder` (str), optional for the first firework  
...
- `max_no_of_ready_jobs` (a positive integer), optional for the first firework.  
...
- `cal_name` (str), **required for every firework**.  
  ...
  Together with `step_no`, a **sub-folder** named `step_ + step_no + _ + cal_name` will be created. Under this folder, the VASP calculation will be carried out. e.g. if `step_no=4`, `cal_name=band str`, then the folder name is `step_4_band_str`  
...
- `move_from_prev_cal`, optional.  
  same as `copy_from_prev_cal`, but those specified files will be moved instead of being copied from the previous firework specified by `copy_which_step`.  
  If `copy_which_step` is not set, "the previous firework" is the *nearest* previous firework.   
...
- `contcar_to_poscar` (bool), optional.  
  Whether to copy the POSCAR from the previous firework specified by `copy_which_step` and rename it as POSCAR.  
  If this tag is `Yes`, the CONTCAR from the previous calculation will be *implicitly* copied and renamed as POSCAR, even though CONTCAR is not in the file list specified by tags `copy_from_prev_cal` or `move_from_prev_cal`  
  **Note that there is no "the previous firework" for the first firework**  
  Default: `No`

--------------------------------

- `copy_which_step`, required except for the first step.  
  This will specify the parent calculation step from which the files listed in `copy_from_prev_cal` are copied or the files listed in `move_from_prev_cal` are moved into the current calculation step.  
...
- `additional_cal_dependence`, optional.  
  ...
  `additional_cal_dependence` should be a full name of an additional dependent calculation step or a array of full names if more than one. In the latter case, separate them using commas `,`.   
    ...
    Note: There are a few HTC tags which may involve the outputs of additional dependent fireworks, such as `EINT_wrt_CBM`, `EINT_wrt_VBM`, `which_step_to_read_cbm_vbm`, `partial_charge_cal`, `NBANDS`, `user_defined_cmd`, `final_user_defined_cmd`, `cmd_to_process_finished_jobs`, `incar_cmd`, `poscar_cmd`, `kpoints_cmd`, `potcar_cmd`. If you set any of these tags, you should also check and set the involved additional dependent steps to `additional_cal_dependence`. Otherwise, an error might be incurred.  
    Default: `empty` 
...
- `extra_copy`, optional.  
  ...
  *If more than one files are specified, separate them with commas.*  
  **The copy action defined by this tag is carried out earlier than the copy/move actions defined by `copy_from_prev_cal`, `move_from_prev_cal`. So it is before the modifications on vasp input files.**  
...
- `comment_incar_tags`, optional.  ***This tag is already obsolete***  
...
- `is_fixed_incar_tags_on`, optional. (**Debug is underway**)   
...
- `fixed_incar_tags`, optional. (**Debug is underway**)  
...
- `partial_charge_cal`, optional.  
  ...
  > LPARD = .TRUE.  
  > NBMOD = -3  
  > EINT  = determined by tag `EINT_wrt_CBM` or|and `EINT_wrt_VBM` and `which_step_to_read_cbm_vbm`

Default: `No`  
**It should be noted that once this tag is set to `Yes`, `EINT_wrt_CBM` or|and `EINT_wrt_VBM` should be set in order to determine `EINT`. `which_step_to_read_cbm_vbm` should also be set to read CBM, VBM and Efermi**

--------------------------------------------

- `which_step_to_read_cbm_vbm`, optional  
  This tag specifies the full name of the previous calculation step (e.g. `step_3_xyz`) where CBM, VBM and Efermi can be read from `step_3_xyz/vasprun.xml` in order to determine `EINT` in INCAR if the partial charge calculation is activated.  
- Format: `step_3_xyz`
- No default value.
- **Notes\:**
    1. `partial_charge_cal` must be set to `Yes` to activate this HTC tag;
    2. `EINT` cannot be determined unless `which_step_to_read_vbm` and one or both of (`EINT_wrt_CBM`, `EINT_wrt_VBM`) are provided at the same time;  
    3. CBM, VBM and Efermi are read from the vasprun.xml of the specified previous step. So that vasprun.xml must exist.  
    4. The calculation step specified via `which_step_to_read_cbm_vbm` will be automatically appended to `additional_cal_dependence`.
    

--------------------------------------------

- `EINT_wrt_CBM` & `EINT_wrt_VBM`, optional    
  **either one or both should be given if `partial_charge_cal=Yes`**  
  Default: `Must be specified in the case where partial charge_cal is set to Yes. No need to set in other cases`  
  **Notes:**  
     1. `partial_charge_cal` must be set to `Yes` to activate these HTC tags;
     2. `EINT` cannot be determined unless `which_step_to_read_vbm` and one or both of (`EINT_wrt_CBM`, `EINT_wrt_VBM`) are provided at the same time  
     3. `EINT` will be internally shifted to be relative to the Fermi level.
     4. When only `EINT_wrt_CBM` (`EINT_wrt_VBM`) is set, the lower and upper bounds of `EINT` is solely determined by that HTC tag;  
     5. When both `EINT_wrt_CBM` and `EINT_wrt_VBM` are set, say `EINT_wrt_VBM=-0.3 0.1` and `EINT_wrt_CBM=-0.05 0.3`, `EINT` is set to the energy window from -0.3 below VBM to 0.3 above CBM.
...
- `EMAX` and `EMIN` setup in `add_new_incar_tags` sub-block, optional  
...
- `NBANDS` in the `add_new_incar_tags` sub-block, optional  
...
- `set_ispin_based_on_prev_cal`, optional (**debug is underway**)  
  ...
  - default: `empty`
  - *Notes*    
    - During the INCAR preparation, INCAR is modified first according to the setup in the `add_new_incar_tags` sub-block. Afterwards, set ISPIN if the current tag is provided. Therefore, if provided, the current tag will overwrite the ISPIN provided in the `add_new_incar_tags` sub-block.  
...
- `ldau_u_j_table`, required if `ldau_cal=Yes`  
...
- `incar_template` (`str`), optional for the first firework  
...
- `valid_incar_tags` (`str`), optional for the first firework  
...
- `kpoints_type`, **case sensitive**, **required for every firework**  
  options - `MPRelaxSet`, `MPStaticSet`, `MPNonSCFSet_line`, `MPNonSCFSet_uniform`, `Line-mode`:
  ...
  - `MPNonSCFSet_line`: The KPOINTS generated by pymatgen.io.vasp.sets.MPNonSCFSet in the line mode --> In pymatgen, this type of KPOINTS is for band structure calculations
...
1. After `extra_copy`, `copy_from_prev_cal` or `move_from_prev_cal`, if there is still no file KPOINTS under the current firework folder, KPOINTS will be created according to `kpoints_type`.   
2. If tag `2d_system` is `Yes`, the KPOINTS will be modified so that K<sub>z</sub> is zero for all kpoints. *Note that this function will be only invoked if KPOINTS under the current firework folder is created. If KPOINTS is copied or moved here from any previous calculations, KPOINTS is then assumed to have zero K<sub>z</sub> for all kpoints.*  
3. Tag `denser_kpoints` is only valid for the KPOINTS generated at `kpoints_type = MPRelaxSet, MPStaticSet`.    

--------------------------------------

- `denser_kpoints` (three float numbers), optional.  
...
- `2d_system`, optional (obsolete in the future)  
  **This tag can be set only in the first firework and this setting will be applied to the whole workflow.**
...
- `sort_structure`, **optional for the first firework**.  
...
`max_ionic_step` (positive integer), optional,  
...
`update_kpoints_every_round` (Boolean: Yes or No), optional.   
...
`skip_this_step`, optional. (Debug is underway)  
...
**Notes**   
  ...
  - Because the step tagged by `skip_this_step` is reserved for later use, it should have no descendant/child calculation steps;
  - Since 8 Aug. 2025, this HTC tag has been improved in the following three aspects:
    1. Only two tags, `step_no` and `cal_name`, are compulsory, in addition to `skip_this_step`;
    2. When this tag is activated, the corresponding calculation folder for ALL to-be-calculated materials will not be created anymore. This makes sense since this calculation step is intentionally skipped for later use. This is different from directly creating signal file `__skipped__` under a calculation folder, which only skips the calculation where it is existent and is thus useful if you just want to skip a calculation step for certain to-be-calculated materials.  
    3. If a calculation step with `skip_this_step=Yes` is found to have any child/desendant calculation steps via either `copy_which_step` or `additional_cal_dependence`, OR its associated calculation folder which in principle should not be created has been already existent, the HTC program will invoke a corresponding error and stop.  
Default: `skip_this_step = No`

-------------------------------------------

`user_defined_cmd`, optional,  
...
`final_user_defined_cmd`, optional,  
...
`incar_cmd`, `poscar_cmd`, `kpoints_cmd` and `potcar_cmd`, optional,   
...
`cmd_to_process_finished_jobs`, optional,   
...
`user_defined_postprocess_cmd`, optional, **obsolete. Replaced by HTC tag `cmd_to_process_finished_jobs` instead**,     
...
`sub_dir_cal_cmd`, required if `sub_dir_cal=Yes`; optional otherwise,  
...
`preview_vasp_inputs`, **optional for the first firework**  
**<ins>This tag has been obsolete.</ins> Not matter what the user sets to `preview_vasp_inputs`, it will eventually be set to `No`. The best way to check whether your `HTC_calculation_setup` works or not should be to feed one small/test structure to it and let the pre-defined calculations run for the test structure.**  
...
- `htc_input_backup` && `htc_input_backup_loc`, optional for the first firework  
...
- `job_submission__script`, **required for every firework**.  
  **Note that the absolute path should be used.**

-------------------------------------------------

- `job_submission_command`, **required for every firework**.  
  e.g. On LSF queue system, suppose the job submission script refered by `job_submission_script` tag is called `vasp.lsf` and this tag can be `bsub < vasp.lsf > job_id`, where `job_id` will store the queue id and will be the value of tag `where_to_parse_queue_id`

--------------------------------

- `job_name`, optional for the first firework.  
...
- `max_running_job` (integer), optional for the first firework    
...
- `re_to_parse_queue_id`, **required only for the first firework**.  
  The regular expression pattern passed to python `re.findall` to parse queue id from the file specified by `where_to_parse_queue_id`.  
  Just make sure this regular expression is well designed such that no other string in the file `where_to_parse_queue_id` can be matched.
...
- `queue_stdout_file_prefix`, `queue_stdout_file_suffix`, `queue_stderr_file_prefix`, `queue_stderr_file_suffix`, **Required for the first firework**.  
  ...
  The two files are useful for their presence indicates that the job is done. If no error is detected for this step, the post-process will proceed and afterwards, the next firework will be carried out. 
     ...
     **Note that you need to specify at least one of the four tags:**  
       ...
       - Sometimes the queue stdout and stderr files will join together, giving a single file. In this case, this single file is the target file. You can set its prefix or suffix using either (`queue_stdout_file_prefix`, `queue_stdout_file_suffix`) or (`queue_stderr_file_prefix`, `queue_stderr_file_suffix`) pair.
...
- `jobs_treated_like_running_jobs`, **optional for the first firework.**   
...
- `__skipped__`: The jobs labeled by this signal file will be skipped. Users can simply judge if a job is necessary via tag `user_defined_cmd`, `final_user_defined_cmd` etc. If the calculation is unnecessary, users can generate this signal file to skip unnecessary calculations. 
  - This signal file is useful if users just aim to skip the calculation steps for <ins>***certain***</ins> materials.   
  - If a calculation step for <ins>***all***</ins> to-be-calculated materials need to be skipped for later use, HTC tag `skip_this_step=Yes` should be set, rather than rely on `__skipped__`.   
  - When a calculation step is skipped, all descendant calculation steps which depend on the skipped step will not be created. Different from what signal file `__stop__` does, the HTC program regards all dependent descendant steps as not being ready for preparation and running. So a material always have some unfinished calculation steps and will never turn into `__complete__` unless the step tagged by `__skipped__` has no dependent descendant steps.  
- `__stop__`: When a calculation step for a material is tagged by `__stop__`, the program will regard the tagged step and all of it descendant steps if any as being unnecessary for that material. Therefore, none of its descendant steps will be created, similar to `__skipped__`. But in this case, the program is able to tag that material with `__complete__` if all other independent steps successfully finished. *Note that when `__stop__` appears under the root directory, it tells the HTC program to stop.*
...
You can stop this program by creating a file named `__stop__` under `${HTC_CWD}` where `python htc_main.py` or `nohup python htc_main.py 2>1&` was executed to start this program. Note that when `__stop__` appears under a calculation step folder of a material, it has a different meaning \(please refer to section [How to control job status](#How-to-control-job-status)\).
